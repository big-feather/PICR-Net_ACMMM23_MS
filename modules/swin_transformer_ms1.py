...
def Backbone_Swin_Transformer(pretrained=True):
    import mindcv
    net = mindcv.create_model('swin_tiny', pretrained=pretrained)
    # net = SwinTransformer()
    # if pretrained:
    ...
    #     pretrained_dict = mindspore.load_checkpoint("./pretrain/swin_tiny-0ff2f96d.ckpt")
    #     # 1. filter out unnecessary keys
    #     filter_dict = {}
    #     for key, value in pretrained_dict.copy().items():
    #         if 'blocks' in key and 'layers'+key[8:] in state_dict:
    #             filter_dict[key] = value
    #             # filter_dict['layers'+key[8:]] = value
    #         if 'blocks' in key and 'layers.'+key in state_dict:
    #             # filter_dict['layers.'+key] = value
    #             filter_dict[key] = value
    #         if 'adam_v' in key and key.replace('adam_v', 'layers') in state_dict:
    #             # filter_dict[key.replace('adam_v', 'layers')] = value
    #             filter_dict[key] = value
    #         if key in state_dict:
    #             filter_dict[key] = value
    #     # pretrained_dict = {k: v for k, v in pretrained_dict.copy().items() if k in state_dict}
    #     print("The number of loaded parameters: ", len(filter_dict))
    #     # 2. overwrite entries in the existing state dict
    #     state_dict.update(filter_dict)
    #
    #     # 3. load the new state dict
    #     mindspore.train.serialization.load_param_into_net(net, state_dict)
    patch_embd = nn.SequentialCell([net.patch_embed,net.pos_drop])
    block_1 = nn.SequentialCell(net.layers[0])
    block_2 = nn.SequentialCell(net.layers[1])
    block_3 = nn.SequentialCell(net.layers[2])
    block_4 = nn.SequentialCell(net.layers[3])

    return patch_embd, block_1, block_2, block_3, block_4


class Swin_T(nn.Cell):
    def __init__(self, pretrained=True):
        ...
        (
            ...
            self.block_3,
            self.block_4,
        ) = Backbone_Swin_Transformer(pretrained=pretrained)

    def construct(self, x):

        patch_embd_feature = self.patch_embd(x)
        # print(patch_embd_feature.shape)
        # print(self.block_1)
        block_1_feature = self.block_1(patch_embd_feature)
        block_2_feature = self.block_2(block_1_feature)
        block_3_feature = self.block_3(block_2_feature)
        block_4_feature = self.block_4(block_3_feature)

        return [patch_embd_feature, block_1_feature,
                block_2_feature, block_3_feature, block_4_feature]
